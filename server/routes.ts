...
const productSchema = z.object({
  ...
  status: z.enum(['in-stock', 'out-of-stock', 'coming-soon']).optional(),
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/products/:id", async (req, res) => {
    try {
      ...
      console.log('🔧 Server: Received update request for product:', productId);
      console.log('🔧 Server: Request body:', JSON.stringify(req.body, null, 2));
      
      // Extract images separately before validation
      const { images, ...updateDataWithoutImages } = req.body;
      
      console.log('🔧 Server: Update data without images:', JSON.stringify(updateDataWithoutImages, null, 2));
      
      // Validate the main product data (without images)
      const validatedData = updateProductSchema.parse(updateDataWithoutImages);
      
      console.log('🔧 Server: Validated update data:', JSON.stringify(validatedData, null, 2));
      ...
      console.log('🔧 Server: Final update data with images:', JSON.stringify(updateWithImages, null, 2));
