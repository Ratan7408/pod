...
const API_BASE = (() => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // In development, always use localhost:5000 for API calls
...
const testUploadEndpoint = async (): Promise<boolean> => {
  try {
    console.log('🔍 Testing upload endpoint:', `${API_BASE}/api/upload/health`);
    ...
    if (!response.ok) {
      console.error('❌ Upload endpoint returned error status:', response.status);
      return false;
    }
    ...
    if (!contentType || !contentType.includes('application/json')) {
      console.error('❌ Upload endpoint returned non-JSON response:', contentType);
      return false;
    }
    
    const result = await response.json();
    console.log('🔍 Upload endpoint test result:', result);
    return true;
  } catch (error) {
...
const checkServerStatus = async (): Promise<{ running: boolean; error?: string }> => {
  try {
    const response = await fetch(`${API_BASE}/api/upload/health`, {
      method: 'GET',
      headers: {
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      return { running: false, error: 'Server is not reachable. Make sure it\'s running on port 5000.' };
    }
...
// ✅ UPDATED: Categories with t-shirt subcategories and status
const CATEGORIES = [
  {
    name: 'T-Shirts',
    available: true,
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!serverStatus.running) {
        throw new Error(`Upload service is not available: ${serverStatus.error}`);
      }
      ...
      if (validUrls.length === 0) {
        throw new Error('No valid images were uploaded. Please try again.');
      }
    ...
    <div className="space-y-4">
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="bg-gray-50 p-3 rounded-lg text-xs">
          ...
          <div>Upload URL: {API_BASE}/api/upload</div>
          <div>Environment: {process.env.NODE_ENV}</div>
          <div className="mt-2 space-y-1">
            <button
              onClick={async () => {
                const available = await testUploadEndpoint();
                alert(`Upload endpoint test: ${available ? 'SUCCESS' : 'FAILED'}`);
              }}
            ...
            <button
              onClick={async () => {
                ...
                alert(`Server Status: ${status.running ? 'RUNNING' : 'NOT RUNNING'}\nError: ${status.error || 'None'}`);
              }}
      ...
      {uploadError && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
          ...
          <div className="mt-2 text-xs text-red-600">
            💡 Tip: If upload continues to fail, you can provide an image URL in the field below instead.
          </div>
        </div>
      )}
...
const ProductForm = ({ product, onSubmit, onCancel, isSubmitting }: {
  ...
  const [formData, setFormData] = useState<Product>({
    ...
    subcategory: product?.subcategory || '', // Initialize subcategory
    status: product?.status || 'in-stock', // Initialize status
  ...
  // ✅ FIXED: Update form data when product prop changes (for editing)
  useEffect(() => {
    if (product) {
      console.log('🔄 Admin: Product prop changed, updating form data:', {
        name: product.name,
      ...
      setFormData({
        ...
        status: product.status || 'in-stock', // Preserve existing status
        inStock: product.inStock ?? true,
        featured: product.featured || false,
        tags: product.tags || '',
        images: initializeImages(product.images)
      });
    }
  }, [product]);
  ...
  const validateForm = (): boolean => {
    ...
    // Validate subcategory for T-Shirts
    if (formData.category === 'T-Shirts' && !formData.subcategory?.trim()) {
      newErrors.subcategory = 'Please select a T-Shirt type';
    }

    // Validate status
    if (!formData.status) {
      newErrors.status = 'Please select a product status';
    }
    ...
    if (!hasUploadedImages && !hasFallbackUrl) {
      newErrors.images = 'At least one product image is required. Please upload photos or provide an image URL.';
    }
  ...
  const handleSubmit = () => {
    ...
    console.log('📤 Admin: Submitting form data:', {
      ...submitData,
      status: submitData.status,
      inStock: submitData.inStock,
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-xs text-gray-500 mt-1">
          Use this if photo upload is not working
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Subcategory Field - Only show for T-Shirts */}
        {formData.category === 'T-Shirts' && (
          <div>
            ...
            >
              ...
              {CATEGORIES.find(cat => cat.name === 'T-Shirts')?.subcategories.map(subcat => (
        ...
        <div>
          ...
          >
            ...
            <option value="coming-soon">🟡 Coming Soon - Not Yet Available</option>
          </select>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      ...
                      {(() => {
                        ...
                        // Try to get the first uploaded image
                        if (product.images) {
                          try {
                            ...
                            if (Array.isArray(parsedImages) && parsedImages.length > 0) {
                              imageSrc = resolveImageUrl(parsedImages[0]);
                              console.log('🖼️ Admin: Using uploaded image:', {
                                original: parsedImages[0],
                                resolved: imageSrc,
                                productName: product.name
                              });
                            }
                          } catch (e) {
                            console.log('🔄 Admin: Failed to parse images for', product.name, ':', e);
                            // If parsing fails, fall back to imageUrl
                          }
                        }
                        
                        // Fall back to imageUrl if no uploaded images
                        if (!imageSrc && product.imageUrl) {
                          imageSrc = resolveImageUrl(product.imageUrl);
                          console.log('🖼️ Admin: Using imageUrl fallback:', {
                        ...
                        if (!imageSrc) {
                          return (
                            <div className="w-12 h-12 bg-gray-200 rounded-lg mr-4 flex items-center justify-center text-xs text-gray-500 font-medium">
                        ...
                        return (
                          <img
                            ...
                            onError={(e) => {
                              ...
                              if (parent) {
                                parent.innerHTML = `
                                  <div class="w-12 h-12 bg-gray-200 rounded-lg mr-4 flex items-center justify-center text-xs text-gray-500 font-medium">
                                    ${product.name.substring(0, 3).toUpperCase()}
                                  </div>
                                `;
                              }
                      ...
                      <div>
                        ...
                        {/* ✅ DEBUG: Show raw image data for troubleshooting */}
                        <div className="text-xs text-gray-500 mt-1">
                          <div>Raw images: {typeof product.images === 'string' ? product.images.substring(0, 50) + '...' : JSON.stringify(product.images).substring(0, 50) + '...'}</div>
                          <div>ImageUrl: {product.imageUrl || 'none'}</div>
                        </div>
                      </div>
                    </div>
                  </td>
...
// ✅ FIXED: Helper function to resolve image URLs (same as frontend)
const resolveImageUrl = (imageUrl: string): string => {
  if (!imageUrl) return '';

  console.log('🔍 Admin: Resolving image URL:', imageUrl);

  // If it's already a complete URL, return as-is
  if (imageUrl.startsWith('http://') || imageUrl.startsWith('https://')) {
    console.log('✅ Admin: Complete URL detected:', imageUrl);
    return imageUrl;
  }

  // Build the base URL - always use localhost:5000 (same as frontend)
  const baseUrl = 'http://localhost:5000';

  // Handle relative URLs starting with /uploads
  if (imageUrl.startsWith('/uploads/')) {
    const resolvedUrl = baseUrl + imageUrl;
    console.log('✅ Admin: Resolved uploads URL:', resolvedUrl);
    return resolvedUrl;
  }
  ...
  if (imageUrl.startsWith('/')) {
    const resolvedUrl = baseUrl + imageUrl;
    console.log('✅ Admin: Resolved relative URL:', resolvedUrl);
    return resolvedUrl;
  }

  // Default case - assume it's a filename and add /uploads/
  const resolvedUrl = `${baseUrl}/uploads/${imageUrl}`;
  console.log('✅ Admin: Resolved filename URL:', resolvedUrl);
  return resolvedUrl;
};
...
export default function AdminDashboard() {
  ...
  const handleUpdateProduct = async (productData: Product) => {
    ...
    setIsSubmitting(true);
    try {
      // ✅ FIXED: Ensure status field is preserved during updates
      const updateData = {
        ...productData,
        status: productData.status || editingProduct.status || 'in-stock' // Preserve status
      };
      
      console.log('🔧 Admin: Updating product with data:', JSON.stringify(updateData, null, 2));
      console.log('🔧 Admin: Original product status:', editingProduct.status);
      console.log('🔧 Admin: New product status:', updateData.status);
